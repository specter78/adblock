...
const SYNTAX_SERIALIZATION_MAP = new Map([
    [exports.AdblockSyntax.Common, 0],
    [exports.AdblockSyntax.Abp, 1],
    [exports.AdblockSyntax.Adg, 2],
    [exports.AdblockSyntax.Ubo, 3],
]);
/**
...
class AgentCommentRuleParser extends ParserBase {
    ...
    static deserialize(buffer, node) {
        ...
        node.type = exports.CommentRuleType.AgentCommentRule;
        node.syntax = exports.AdblockSyntax.Common;
        node.category = exports.RuleCategory.Comment;
        let prop = buffer.readUint8();
        while (prop !== NULL) {
            switch (prop) {
...
class ConfigCommentRuleParser extends ParserBase {
    ...
    static deserialize(buffer, node) {
        ...
        node.type = exports.CommentRuleType.ConfigCommentRule;
        node.category = exports.RuleCategory.Comment;
        node.syntax = exports.AdblockSyntax.Common;
        let prop = buffer.readUint8();
        while (prop !== NULL) {
...
class HintCommentRuleParser extends ParserBase {
    ...
    static deserialize(buffer, node) {
        ...
        node.category = exports.RuleCategory.Comment;
        node.type = exports.CommentRuleType.HintCommentRule;
        let prop = buffer.readUint8();
        while (prop !== NULL) {
            switch (prop) {
                case HintRuleSerializationMap.Syntax:
                    node.syntax = SYNTAX_DESERIALIZATION_MAP.get(buffer.readUint8()) ?? exports.AdblockSyntax.Common;
                    break;
...
class MetadataCommentRuleParser extends ParserBase {
    ...
    static deserialize(buffer, node) {
        ...
        node.type = exports.CommentRuleType.MetadataCommentRule;
        node.category = exports.RuleCategory.Comment;
        node.syntax = exports.AdblockSyntax.Common;
        let prop = buffer.readUint8();
        while (prop !== NULL) {
...
class PreProcessorCommentRuleParser extends ParserBase {
    ...
    static deserialize(buffer, node) {
        ...
        node.type = exports.CommentRuleType.PreProcessorCommentRule;
        node.category = exports.RuleCategory.Comment;
        node.syntax = exports.AdblockSyntax.Common;
        let prop = buffer.readUint8();
        while (prop !== NULL) {
...
class SimpleCommentParser extends ParserBase {
    ...
    static isSimpleComment(raw) {
        ...
        if (trimmed.startsWith(exports.CommentMarker.Regular)) {
            return true;
        }
    ...
    static deserialize(buffer, node) {
        ...
        node.type = exports.CommentRuleType.CommentRule;
        node.category = exports.RuleCategory.Comment;
        node.syntax = exports.AdblockSyntax.Common;
        let prop = buffer.readUint8();
        while (prop !== NULL) {
...
const standardTokenPairs = new Map([
    [cssTokenizer.TokenType.Function, cssTokenizer.TokenType.CloseParenthesis],
    [cssTokenizer.TokenType.OpenParenthesis, cssTokenizer.TokenType.CloseParenthesis],
    [cssTokenizer.TokenType.OpenSquareBracket, cssTokenizer.TokenType.CloseSquareBracket],
    [cssTokenizer.TokenType.OpenCurlyBracket, cssTokenizer.TokenType.CloseCurlyBracket],
]);
/**
...
const tokenizeWithBalancedPairs = (raw, onToken, onError = () => { }, functionHandlers, tokenPairs = standardTokenPairs) => {
    ...
    cssTokenizer.tokenizeExtended(raw, (type, start, end, props) => {
        if (tokenPairs.has(type)) {
            ...
            if (stack[stack.length - 1] === type) {
                ...
                throw new AdblockSyntaxError(sprintfJs.sprintf(ERROR_MESSAGES$6.EXPECTED_TOKEN_BUT_GOT, cssTokenizer.getFormattedTokenName(stack[stack.length - 1]), cssTokenizer.getFormattedTokenName(type)), start, raw.length);
            }
        }
    ...
    if (stack.length > 0) {
        throw new AdblockSyntaxError(sprintfJs.sprintf(ERROR_MESSAGES$6.EXPECTED_TOKEN_BUT_GOT, cssTokenizer.getFormattedTokenName(stack[stack.length - 1]), END_OF_INPUT), raw.length - 1, raw.length);
    }
};
/**
    ...
    static parse(raw, options = defaultParserOptions, baseOffset = 0) {
        ...
        const stackModifier = (modifier) => {
            if (processedModifiers.has(modifier.name)) {
                throw new AdblockSyntaxError(sprintfJs.sprintf(ERROR_MESSAGES$5.DUPLICATED_UBO_MODIFIER, formatPseudoName(modifier.name)), baseOffset + modifier.modifierStart, baseOffset + raw.length);
            }
        ...
        tokenizeFnBalanced(raw, (type, start, end, _, balance) => {
            ...
            if ((processedModifiers.has(UboPseudoName.Style)
                ...
                && type !== cssTokenizer.TokenType.Whitespace) {
            ...
            if (tokens[i - 1]?.type === cssTokenizer.TokenType.Colon && type === cssTokenizer.TokenType.Function) {
                    ...
                    if (balance > 1) {
                        ...
                        if (fn === UboPseudoName.MatchesPath) {
                            if (uboModifierStack.length > 0) {
                                throw new AdblockSyntaxError(sprintfJs.sprintf(ERROR_MESSAGES$5.PSEUDO_CANNOT_BE_NESTED, formatPseudoName(UboPseudoName.MatchesPath), formatPseudoName(uboModifierStack[uboModifierStack.length - 1].name), formatPseudoName(CSS_NOT_PSEUDO)), baseOffset + start - 1, baseOffset + raw.length);
                            }
                            ...
                            for (let j = i - 1; j >= 0; j -= 1) {
                                ...
                                if (tokens[j].balance === 0) {
                                ...
                                else if (tokens[j].type === cssTokenizer.TokenType.Colon
                                    || tokens[j].type === cssTokenizer.TokenType.Whitespace) {
                                    continue;
                                }
                                else if (tokens[j].type === cssTokenizer.TokenType.Function) {
                                    const wrapperFnName = raw.slice(tokens[j].start, tokens[j].end - 1);
                                    if (wrapperFnName !== CSS_NOT_PSEUDO) {
                                        throw new AdblockSyntaxError(sprintfJs.sprintf(ERROR_MESSAGES$5.PSEUDO_CANNOT_BE_NESTED, formatPseudoName(UboPseudoName.MatchesPath), formatPseudoName(wrapperFnName), formatPseudoName(CSS_NOT_PSEUDO)), baseOffset + tokens[j].start - 1, baseOffset + raw.length);
                                    }
                                    if (tokens[j - 1]?.type !== cssTokenizer.TokenType.Colon) {
                                        const got = tokens[j - 1]?.type
                                            ? cssTokenizer.getFormattedTokenName(tokens[j - 1]?.type)
                                            : 'nothing';
                                        throw new AdblockSyntaxError(sprintfJs.sprintf(ERROR_MESSAGES$5.EXPECTED_BUT_GOT_BEFORE, cssTokenizer.getFormattedTokenName(cssTokenizer.TokenType.Colon), got, formatPseudoName(UboPseudoName.MatchesPath, CSS_NOT_PSEUDO)), 
                                    ...
                                    throw new AdblockSyntaxError(sprintfJs.sprintf(ERROR_MESSAGES$5.NEGATED_UBO_MODIFIER_CANNOT_BE_PRECEDED_BY, formatPseudoName(UboPseudoName.MatchesPath), cssTokenizer.getFormattedTokenName(tokens[j].type)), baseOffset + tokens[j].start, baseOffset + raw.length);
                                }
                            }
                            stackModifier({
                                name: fn,
                            ...
                            throw new AdblockSyntaxError(sprintfJs.sprintf(ERROR_MESSAGES$5.UBO_MODIFIER_CANNOT_BE_NESTED, formatPseudoName(fn)), baseOffset + start - 1, baseOffset + raw.length);
                        }
                    }
                    else {
                        stackModifier({
                ...
                if (lastStackedModifier?.name === UboPseudoName.MatchesPath && lastStackedModifier?.isException) {
                    if (!(type === cssTokenizer.TokenType.CloseParenthesis || type === cssTokenizer.TokenType.Whitespace)
                        && balance < lastStackedModifier.valueBalance) {
                        throw new AdblockSyntaxError(sprintfJs.sprintf(ERROR_MESSAGES$5.NEGATED_UBO_MODIFIER_CANNOT_BE_FOLLOWED_BY, formatPseudoName(UboPseudoName.MatchesPath), cssTokenizer.getFormattedTokenName(type)), baseOffset + start, baseOffset + raw.length);
                    }
                }
                ...
                if (type === cssTokenizer.TokenType.CloseParenthesis && lastStackedModifier) {
...
class CssTokenStream {
    ...
    getOrFail(index = this.index) {
        ...
        if (!token) {
            throw new AdblockSyntaxError(sprintfJs.sprintf(ERROR_MESSAGES$6.EXPECTED_ANY_TOKEN_BUT_GOT, END_OF_INPUT), this.baseOffset + this.source.length - 1, this.baseOffset + this.source.length);
        }
        return token;
    }
    /**
    ...
    expect(type, data) {
        ...
        if (!token) {
            throw new AdblockSyntaxError(sprintfJs.sprintf(ERROR_MESSAGES$6.EXPECTED_TOKEN_BUT_GOT, cssTokenizer.getFormattedTokenName(type), END_OF_INPUT), this.baseOffset + this.source.length - 1, this.baseOffset + this.source.length);
        }
        if (token.type !== type) {
            throw new AdblockSyntaxError(sprintfJs.sprintf(ERROR_MESSAGES$6.EXPECTED_TOKEN_BUT_GOT, cssTokenizer.getFormattedTokenName(type), cssTokenizer.getFormattedTokenName(token.type)), this.baseOffset + token.start, this.baseOffset + token.end);
        }
        if (data?.balance !== undefined && token.balance !== data.balance) {
            throw new AdblockSyntaxError(sprintfJs.sprintf(ERROR_MESSAGES$6.EXPECTED_TOKEN_WITH_BALANCE_BUT_GOT, cssTokenizer.getFormattedTokenName(type), data.balance, token.balance), this.baseOffset + token.start, this.baseOffset + token.end);
        }
        if (data?.value && this.fragment() !== data.value) {
            throw new AdblockSyntaxError(sprintfJs.sprintf(ERROR_MESSAGES$6.EXPECTED_TOKEN_WITH_VALUE_BUT_GOT, cssTokenizer.getFormattedTokenName(type), data.value, this.fragment()), this.baseOffset + token.start, this.baseOffset + token.end);
        }
    }
    /**
...
class AdgCssInjectionParser extends ParserBase {
    ...
    static parse(raw, options = defaultParserOptions, baseOffset = 0) {
        ...
        // Handle '@media' case:
        let balanceShift = 0;
        if (stream.getOrFail().type === cssTokenizer.TokenType.AtKeyword) {
            // Currently only '@media' is supported, we should throw an error if we encounter anything else,
            // like '@supports' or '@charset', etc.
            stream.expect(cssTokenizer.TokenType.AtKeyword, { value: CSS_MEDIA_MARKER, balance: 0 });
            stream.advance();
            ...
            while (!stream.isEof()) {
                ...
                if (token.type === cssTokenizer.TokenType.OpenCurlyBracket && token.balance === 1) {
                    break;
                }
                if (token.type !== cssTokenizer.TokenType.Whitespace) {
        ...
        //           ↑
        //           └ this one
        const { skippedTrimmed: selectorTokensLength } = stream.skipUntilExt(cssTokenizer.TokenType.OpenCurlyBracket, balanceShift + 1);
        stream.expect(cssTokenizer.TokenType.OpenCurlyBracket);
        ...
        while (!stream.isEof()) {
            const token = stream.getOrFail();
            if (token.type === cssTokenizer.TokenType.CloseCurlyBracket && stream.getBalance() === balanceShift) {
            ...
            if (token.type === cssTokenizer.TokenType.Ident && stream.lookahead()?.type === cssTokenizer.TokenType.Colon) {
                ...
                if (ident === REMOVE_PROPERTY) {
                    ...
                    stream.expect(cssTokenizer.TokenType.Ident, { value: REMOVE_VALUE });
                    stream.advance();
                    remove = true;
                }
            }
        ...
        //                                                      ↑
        //                                                      └ this one
        stream.expect(cssTokenizer.TokenType.CloseCurlyBracket);
        stream.advance();
    ...
    static parse(raw, options = defaultParserOptions, baseOffset = 0) {
        ...
        while (offset < closingParenthesesIndex) {
            ...
            if (parameterList.children.length > 0) {
                if (raw[offset] !== COMMA) {
                    throw new AdblockSyntaxError(sprintfJs.sprintf(AdgScriptletInjectionBodyParser.ERROR_MESSAGES.EXPECTED_COMMA, raw[offset]), baseOffset + offset, baseOffset + raw.length);
                }
                // Eat the comma
                offset += 1;
                // Skip whitespace
            ...
            if (raw[offset] === SINGLE_QUOTE || raw[offset] === DOUBLE_QUOTE) {
                ...
                throw new AdblockSyntaxError(sprintfJs.sprintf(AdgScriptletInjectionBodyParser.ERROR_MESSAGES.EXPECTED_QUOTE, raw[offset]), baseOffset + offset, baseOffset + raw.length);
            }
        }
...
const COSMETIC_RULE_TYPE_DESERIALIZATION_MAP = new Map([
    [BinaryTypeMap.ElementHidingRule, exports.CosmeticRuleType.ElementHidingRule],
    [BinaryTypeMap.CssInjectionRule, exports.CosmeticRuleType.CssInjectionRule],
    [BinaryTypeMap.ScriptletInjectionRule, exports.CosmeticRuleType.ScriptletInjectionRule],
    [BinaryTypeMap.JsInjectionRule, exports.CosmeticRuleType.JsInjectionRule],
    [BinaryTypeMap.HtmlFilteringRule, exports.CosmeticRuleType.HtmlFilteringRule],
]);
/**
 * Property map for binary serialization. This helps to reduce the size of the serialized data,
 * as it allows us to use a single byte to represent a property.
...
class CosmeticRuleParser extends ParserBase {
    ...
    static parse(raw, options = defaultParserOptions, baseOffset = 0) {
        ...
        if (rawPattern[patternOffset] === OPEN_SQUARE_BRACKET) {
            ...
            if (rawPattern[patternOffset] !== DOLLAR_SIGN) {
                throw new AdblockSyntaxError(sprintfJs.sprintf(ERROR_MESSAGES$3.MISSING_ADGUARD_MODIFIER_LIST_MARKER, DOLLAR_SIGN, rawPattern), baseOffset + patternOffset, baseOffset + rawPattern.length);
            }
            ...
            if (modifierListEnd === -1) {
                throw new AdblockSyntaxError(sprintfJs.sprintf(ERROR_MESSAGES$3.MISSING_ADGUARD_MODIFIER_LIST_END, CLOSE_SQUARE_BRACKET, rawPattern), baseOffset + patternOffset, baseOffset + rawPattern.length);
            }
            // Parse modifier list
        ...
        const expectCommonOrSpecificSyntax = (expectedSyntax) => {
            if (syntax !== exports.AdblockSyntax.Common && syntax !== expectedSyntax) {
                throw new AdblockSyntaxError(sprintfJs.sprintf(ERROR_MESSAGES$3.SYNTAXES_CANNOT_BE_MIXED, expectedSyntax, syntax), baseOffset + patternStart, baseOffset + bodyEnd);
            }
        };
        let uboSelector;
        ...
        if (options.parseUboSpecificRules) {
            ...
            if (uboSelector.modifiers && uboSelector.modifiers.children.length > 0) {
                ...
                expectCommonOrSpecificSyntax(exports.AdblockSyntax.Ubo);
        ...
        const parseAdgCssInjection = () => {
            ...
            expectCommonOrSpecificSyntax(exports.AdblockSyntax.Adg);
            return {
                syntax: exports.AdblockSyntax.Adg,
                type: exports.CosmeticRuleType.CssInjectionRule,
        ...
        const parseAbpSnippetInjection = () => {
            ...
            expectCommonOrSpecificSyntax(exports.AdblockSyntax.Abp);
        ...
        const parseUboScriptletInjection = () => {
            ...
            if (!options.parseUboSpecificRules) {
                throw new AdblockSyntaxError(sprintfJs.sprintf(ERROR_MESSAGES$3.SYNTAX_DISABLED, exports.AdblockSyntax.Ubo), baseOffset + bodyStart, baseOffset + bodyEnd);
            }
            expectCommonOrSpecificSyntax(exports.AdblockSyntax.Ubo);
        ...
        if (!restProps) {
            throw new AdblockSyntaxError(sprintfJs.sprintf(ERROR_MESSAGES$3.INVALID_BODY_FOR_SEPARATOR, rawBody, separatorResult.separator), baseOffset + bodyStart, baseOffset + bodyEnd);
        }
    ...
    static generatePattern(node) {
        ...
        if (node.syntax === exports.AdblockSyntax.Adg && node.modifiers && node.modifiers.children.length > 0) {
    ...
    static generateBody(node) {
        ...
        switch (node.type) {
            ...
            case exports.CosmeticRuleType.ScriptletInjectionRule:
                switch (node.syntax) {
                    case exports.AdblockSyntax.Adg:
    ...
    static generate(node) {
        ...
        // uBO rule modifiers
        if (node.syntax === exports.AdblockSyntax.Ubo && node.modifiers) {
    ...
    static deserialize(buffer, node) {
        ...
        node.type = type;
        node.category = exports.RuleCategory.Cosmetic;
        const syntax = SYNTAX_DESERIALIZATION_MAP.get(buffer.readUint8()) ?? exports.AdblockSyntax.Common;
        node.syntax = syntax;
...
class NetworkRuleParser extends ParserBase {
    ...
    static deserialize(buffer, node) {
        ...
        node.type = exports.NetworkRuleType.NetworkRule;
        node.category = exports.RuleCategory.Network;
        node.modifiers = undefined;
        let prop = buffer.readUint8();
        while (prop !== NULL) {
            switch (prop) {
...
class HostRuleParser extends ParserBase {
    ...
    static parse(raw, options = defaultParserOptions, baseOffset = 0) {
        ...
        const result = {
            category: exports.RuleCategory.Network,
            type: exports.NetworkRuleType.HostRule,
            syntax: exports.AdblockSyntax.Common,
        };
        if (partsLength === 1) {
    ...
    static deserialize(buffer, node) {
        ...
        node.category = exports.RuleCategory.Network;
        node.type = exports.NetworkRuleType.HostRule;
        let prop = buffer.readUint8();
        while (prop !== NULL) {
            switch (prop) {
...
class RuleParser extends ParserBase {
    ...
    static generate(ast) {
        switch (ast.category) {
            ...
            case exports.RuleCategory.Network:
                switch (ast.type) {
                    case exports.NetworkRuleType.HostRule:
                        return HostRuleParser.generate(ast);
                    case exports.NetworkRuleType.NetworkRule:
    ...
    static deserializeEmptyRule(buffer, node) {
        ...
        node.type = 'EmptyRule';
        node.category = exports.RuleCategory.Empty;
        node.syntax = exports.AdblockSyntax.Common;
        let prop = buffer.readUint8();
        while (prop !== NULL) {
            switch (prop) {
    ...
    static deserializeInvalidRule(buffer, node) {
        ...
        node.category = exports.RuleCategory.Invalid;
        let prop = buffer.readUint8();
        while (prop !== NULL) {
            switch (prop) {
 ...
 */
exports.SpecificPlatform = void 0;
...
})(exports.SpecificPlatform || (exports.SpecificPlatform = {}));
/**
 * List of generic platforms (combinations of specific platforms).
 */
exports.GenericPlatform = void 0;
...
})(exports.GenericPlatform || (exports.GenericPlatform = {}));
...
const SPECIFIC_PLATFORM_MAP = new Map([
    ['adg_os_windows', exports.SpecificPlatform.AdgOsWindows],
    ['adg_os_mac', exports.SpecificPlatform.AdgOsMac],
    ['adg_os_android', exports.SpecificPlatform.AdgOsAndroid],
    ['adg_ext_chrome', exports.SpecificPlatform.AdgExtChrome],
    ['adg_ext_opera', exports.SpecificPlatform.AdgExtOpera],
    ['adg_ext_edge', exports.SpecificPlatform.AdgExtEdge],
    ['adg_ext_firefox', exports.SpecificPlatform.AdgExtFirefox],
    ['adg_cb_android', exports.SpecificPlatform.AdgCbAndroid],
    ['adg_cb_ios', exports.SpecificPlatform.AdgCbIos],
    ['adg_cb_safari', exports.SpecificPlatform.AdgCbSafari],
    ['ubo_ext_chrome', exports.SpecificPlatform.UboExtChrome],
    ['ubo_ext_opera', exports.SpecificPlatform.UboExtOpera],
    ['ubo_ext_edge', exports.SpecificPlatform.UboExtEdge],
    ['ubo_ext_firefox', exports.SpecificPlatform.UboExtFirefox],
    ['abp_ext_chrome', exports.SpecificPlatform.AbpExtChrome],
    ['abp_ext_opera', exports.SpecificPlatform.AbpExtOpera],
    ['abp_ext_edge', exports.SpecificPlatform.AbpExtEdge],
    ['abp_ext_firefox', exports.SpecificPlatform.AbpExtFirefox],
]);
/**
...
const GENERIC_PLATFORM_MAP = new Map([
    ['adg_os_any', exports.GenericPlatform.AdgOsAny],
    ['adg_safari_any', exports.GenericPlatform.AdgSafariAny],
    ['adg_ext_chromium', exports.GenericPlatform.AdgExtChromium],
    ['adg_ext_any', exports.GenericPlatform.AdgExtAny],
    ['adg_any', exports.GenericPlatform.AdgAny],
    ['ubo_ext_chromium', exports.GenericPlatform.UboExtChromium],
    ['ubo_ext_any', exports.GenericPlatform.UboExtAny],
    ['ubo_any', exports.GenericPlatform.UboAny],
    ['abp_ext_chromium', exports.GenericPlatform.AbpExtChromium],
    ['abp_ext_any', exports.GenericPlatform.AbpExtAny],
    ['abp_any', exports.GenericPlatform.AbpAny],
    ['any', exports.GenericPlatform.Any],
]);
/**
...
class CompatibilityTableBase {
    ...
    getRowsByProduct() {
        ...
        for (let i = 0; i < this.data.shared.length; i += 1) {
            ...
            const keys = Object.keys(data.map);
            const row = {
                [exports.AdblockSyntax.Adg]: {},
                [exports.AdblockSyntax.Ubo]: {},
                [exports.AdblockSyntax.Abp]: {},
            };
            for (let j = 0; j < keys.length; j += 1) {
                const key = Number(keys[j]);
                if (key & exports.GenericPlatform.AdgAny) {
                    row[exports.AdblockSyntax.Adg][key] = data.shared[data.map[key]];
                }
                else if (key & exports.GenericPlatform.UboAny) {
                    row[exports.AdblockSyntax.Ubo][key] = data.shared[data.map[key]];
                }
                else if (key & exports.GenericPlatform.AbpAny) {
                    row[exports.AdblockSyntax.Abp][key] = data.shared[data.map[key]];
                }
            }
            result.push(row);
        }
 ...
 */
deepFreeze(compatibilityTableData_js.modifiersCompatibilityTableData);
/**
 * Compatibility table instance for modifiers.
 */
const modifiersCompatibilityTable = new ModifiersCompatibilityTable(compatibilityTableData_js.modifiersCompatibilityTableData);
...
const convertSyntaxToGenericPlatform = (syntax) => {
    switch (syntax) {
        case exports.AdblockSyntax.Adg:
            return exports.GenericPlatform.AdgAny;
        case exports.AdblockSyntax.Ubo:
            return exports.GenericPlatform.UboAny;
        case exports.AdblockSyntax.Abp:
            return exports.GenericPlatform.AbpAny;
        default:
...
class HtmlRuleConverter extends RuleConverterBase {
    ...
    static convertToAdg(rule) {
        ...
        stream.skipWhitespace();
        // Skip ^
        stream.expect(cssTokenizer.TokenType.Delim, { value: UBO_HTML_MASK });
        stream.advance();
        while (!stream.isEof()) {
            const token = stream.getOrFail();
            if (token.type === cssTokenizer.TokenType.Ident) {
                ...
                if (convertedSelector.length !== 0 && stream.lookbehindForNonWs() !== undefined) {
                    throw new RuleConversionError(sprintfJs.sprintf(ERROR_MESSAGES$2.TAG_SHOULD_BE_FIRST_CHILD, cssTokenizer.getFormattedTokenName(token.type), source.slice(token.start, token.end)));
                }
                ...
                if (tempToken.type !== cssTokenizer.TokenType.Ident) {
                    throw new RuleConversionError(sprintfJs.sprintf(ERROR_MESSAGES$2.INVALID_ATTRIBUTE_NAME, cssTokenizer.getFormattedTokenName(tempToken.type), source.slice(tempToken.start, tempToken.end)));
                }
                ...
                if (tempToken.type === cssTokenizer.TokenType.CloseSquareBracket) {
                    ...
                    if (attr === AttributeSelectors.MinLength || attr === AttributeSelectors.MaxLength) {
                        throw new RuleConversionError(sprintfJs.sprintf(ERROR_MESSAGES$2.ATTRIBUTE_SELECTOR_REQUIRES_VALUE, attr));
                    }
                ...
                stream.expect(cssTokenizer.TokenType.Delim, { value: EQUALS });
                ...
                if (tempToken.type !== cssTokenizer.TokenType.Ident && tempToken.type !== cssTokenizer.TokenType.String) {
                    throw new RuleConversionError(sprintfJs.sprintf(ERROR_MESSAGES$2.INVALID_ATTRIBUTE_VALUE, cssTokenizer.getFormattedTokenName(tempToken.type), source.slice(tempToken.start, tempToken.end)));
                }
                ...
                tempToken = stream.getOrFail();
                if (tempToken.type === cssTokenizer.TokenType.Ident) {
                    throw new RuleConversionError(sprintfJs.sprintf(ERROR_MESSAGES$2.FLAGS_NOT_SUPPORTED));
                }
                ...
                if (attr === AttributeSelectors.MinLength) {
                    ...
                    if (Number.isNaN(parsed)) {
                        throw new RuleConversionError(sprintfJs.sprintf(ERROR_MESSAGES$2.VALUE_FOR_ATTR_SHOULD_BE_INT, attr, value));
                    }
                    minLen = parsed;
                }
                ...
                if (!SUPPORTED_UBO_PSEUDO_CLASSES.has(fn)) {
                    throw new RuleConversionError(sprintfJs.sprintf(ERROR_MESSAGES$2.INVALID_PSEUDO_CLASS, fn));
                }
                ...
                if (fn === PseudoClasses$1.MinTextLength) {
                    ...
                    if (Number.isNaN(parsed)) {
                        throw new RuleConversionError(sprintfJs.sprintf(ERROR_MESSAGES$2.VALUE_FOR_PSEUDO_CLASS_SHOULD_BE_INT, fn, param));
                    }
                    minLen = parsed;
                }
                    ...
                    if (RegExpUtils.isRegexPattern(param)) {
                        ...
                        throw new RuleConversionError(sprintfJs.sprintf(ERROR_MESSAGES$2.REGEXP_NOT_SUPPORTED, param, fn));
                    }
                ...
                throw new RuleConversionError(sprintfJs.sprintf(ERROR_MESSAGES$2.UNEXPECTED_TOKEN_WITH_VALUE, cssTokenizer.getFormattedTokenName(token.type), source.slice(token.start, token.end)));
            }
        }
 ...
 * Deep freeze the compatibility table data to avoid accidental modifications.
 */
deepFreeze(compatibilityTableData_js.redirectsCompatibilityTableData);
/**
 * Compatibility table instance for redirects.
 */
const redirectsCompatibilityTable = new RedirectsCompatibilityTable(compatibilityTableData_js.redirectsCompatibilityTableData);
 ...
 * Deep freeze the compatibility table data to avoid accidental modifications.
 */
deepFreeze(compatibilityTableData_js.scriptletsCompatibilityTableData);
/**
 * Compatibility table instance for scriptlets.
 */
const scriptletsCompatibilityTable = new ScriptletsCompatibilityTable(compatibilityTableData_js.scriptletsCompatibilityTableData);
        ...
        modifierList.children.forEach((modifierNode, index) => {
            ...
            if (REDIRECT_MODIFIERS.has(modifierNode.name.value)) {
                ...
                const convertedRedirectResource = redirectResource
                    ? redirectsCompatibilityTable.getFirst(redirectResource, exports.GenericPlatform.AdgAny)?.name
                    : undefined;
    ...
    static convertToAdg(rule) {
        ...
        switch (rule.category) {
            ...
            case exports.RuleCategory.Network:
                ...
                if (rule.type === exports.NetworkRuleType.HostRule) {
...
exports.AGTREE_VERSION = AGTREE_VERSION;
...
exports.BINARY_SCHEMA_VERSION = BINARY_SCHEMA_VERSION;
exports.BinarySchemaMismatchError = BinarySchemaMismatchError;
...
exports.KNOWN_METADATA_HEADERS = KNOWN_METADATA_HEADERS;
...
exports.OutputByteBuffer = OutputByteBuffer;
...
exports.decodeTextPolyfill = decodeTextPolyfill;
...
exports.redirectsCompatibilityTable = redirectsCompatibilityTable;
exports.scriptletsCompatibilityTable = scriptletsCompatibilityTable;
