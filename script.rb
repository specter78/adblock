  ...
  if capture = /^(?:@@)?(?:\|\|?)?(?:https?)?(?:\:\/\/)?([^#^\^^$^%]+)(.*)/.match(line)
    ...
    if /optimized/.match(filename) # filter list optimization
      ...
      if /(?:annoyances|social)/.match(filename)
        ...
        return true if /\.(?:ru|de|jp|pl|ua|tr|it|fr|br|nl|by|es|kr|cn|cz|hu|no|ch|at|tw|gr|au|be|ro|se|pt|sk|vn|kz|fi|ca|dk|lv|hr|ar|ee|uk|me|fm|id|xyz|eu|co|info|news|biz|net|\*)$/.match(domain) # tlds in annoyances and social
      end
    end
